...
class NASBench201DatasetLatency(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for net_type in ['cifar10-valid', 'cifar100', 'ImageNet16-120']:
            ...
            if net_type == 'cifar10-valid':
                ...
                latency = arch['cifar10_latency']

                network_data['cifar10'] = (arch_feature[net_type], val_acc, test_acc, params, flops, edges_type_counts, rank, latency)
            elif net_type == 'ImageNet16-120':
                val_acc = arch['imagenet16_val_acc']
                test_acc = arch['imagenet16_test_acc']
                rank = arch['imagenet16_rank']
                latency = arch['imagenet16_latency']
                
                network_data['imagenet16'] = (arch_feature[net_type], val_acc, test_acc, params, flops, edges_type_counts, rank, latency)
            else:
                val_acc = arch['cifar100_val_acc']
                test_acc = arch['cifar100_test_acc']
                rank = arch['cifar100_rank']
                latency = arch['cifar100_latency']

                network_data['cifar100'] = (arch_feature[net_type], val_acc, test_acc, params, flops, edges_type_counts, rank, latency)
