...
from dataset import NASBench201DatasetLatency, SplitSubet201
from sampler import ArchSampler201Latency 

from .train_utils import *
from utils.metric import compute_lat_acc

def evaluate_sampled_batch_201_latency(model, sampler : ArchSampler201Latency, tier_list, batch_statics_dict, dataset : NASBench201DatasetLatency, it, args, device, writer, logger, flag):

    lat_cr = torch.nn.MSELoss().cuda(device)
    ...
    for _, batch in enumerate(sampled_dataloader):
        ...
        t1_latency = latency[idx]

        t1_pred_val_acc =  val_acc_pred.squeeze(1)[idx]
        t1_pred_latency = latency_pred.squeeze(1)[idx]
        ...
        if t1_pred_val_acc.size(0) >= 5:
            ...
            tpk5_latency = t1_latency[tpk5_idx]
            tpk5_pred_lat = t1_pred_latency[tpk5_idx]
            tpk5_lat_rmse = torch.sqrt(lat_cr(tpk5_pred_lat, tpk5_latency))
            tpk5_lat_acc5 = compute_lat_acc(tpk5_pred_lat, tpk5_latency, threshold=0.05)
            tpk5_lat_acc10 = compute_lat_acc(tpk5_pred_lat, tpk5_latency, threshold=0.1)
            ...
            tpk1_latency = t1_latency[tpk1_idx]
            tpk1_pred_lat = t1_pred_latency[tpk1_idx]
            tpk1_lat_rmse = torch.sqrt(lat_cr(tpk1_pred_lat, tpk1_latency))


            results_tpk1.append((tpk1_test_acc.item(), tpk1_rank.item(), tpk1_val_acc.item(), tpk1_lat_rmse))
            results_tpk5.append((tpk5_best_test_acc.item(), tpk5_best_rank.item(), tpk5_best_val.item(), tpk5_lat_rmse, tpk5_lat_acc5, tpk5_lat_acc10))
        
    if len(results_tpk1) != 0 or len(results_tpk5) != 0:
        (best_acc_at1, best_rank_at1, best_val_acc_at1, best_lat_rmse_at1) = sorted(results_tpk1, key=lambda item:item[0], reverse=True)[0]
        (best_acc_at5, best_rank_at5, best_val_acc_at5, best_lat_rmse_at5, best_lat_acc5_at5, best_lat_acc10_at5) = sorted(results_tpk5, key=lambda item:item[0], reverse=True)[0]
        ...
        logger.info('[{}][iter:{:2d}] Time: {:.2f} Test Acc@top5: {:.8f} Rank: {:5d}(top {:6.2%}) Val Acc@top5: {:.8f}| Test Acc@top1: {:.8f} Rank: {:5d}(top{:6.2%}) Val Acc@top5: {:.6f} RMSE@top5: {:.6f}, Acc5@top5: {:.6f}, Acc10@top5: {:.6f}, RMSE@top1: {:.4f}'.format(
        ...
        return best_acc_at1, best_rank_at1, best_val_acc_at1, best_acc_at5, best_rank_at5, best_val_acc_at5, best_lat_rmse_at5, best_lat_acc5_at5, best_lat_acc10_at5, best_lat_rmse_at1
